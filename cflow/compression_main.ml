...
let compress_and_print logger env log_info step_list =
  ...
  if (not causal_trace_on) && (not weak_compression_on) && (not strong_compression_on)
  ...
  begin 
	...
	if not (U.has_obs step_list) then let () = Debug.tag logger "+ No causal flow found" in empty_compression
  else
    let () =
    ...
    in
      ...
      let () = if debug_mode && log_step then Debug.tag logger "\t - pretty printing the grid"in 
      let error = if debug_mode then 
      ...
      let () = if log_step then Format.fprintf logger "\t - computing causal past of each observed events (%i)@." n_stories in
	    let error,log_info,causal_story_list = 
			  let () = if debug_mode then Debug.tag logger "\t\t * causal compression " in 
			  ...
			  let _ = if Parameter.log_number_of_causal_flows then 
          Causal.print_stat logger parameter handler enriched_grid 
			  in 
				let () =
				  if log_step 
				  then Format.fprintf logger "\t - %s (%i)@." (
				  	if store_uncompressed_stories then "causal flow compression"
				    else "causal & weak flow compression"
				  ) n_stories 
				in
          ...
          (fun (error,log_info,story_list) observable_id -> 
            ...
            let info = match U.get_runtime_info_from_observable_hit observable_id 
                    with 
                    | None -> []
                    | Some info -> 
            ...
            in
	  			...
	  			) (error,log_info,table1) (List.rev list)
	    in 
    ...
    in
	  ...
	  let one_iteration_of_compression (log_info,error,event_list) = 
	    let error,log_info,event_list = 
	      ...
	      then U.fill_siphon parameter always handler log_info error event_list 
	      else error,log_info,event_list 
	    in
	    let () = if debug_mode then U.print_trace parameter handler event_list in
	    let error,log_info,event_list = 
	      if  Parameter.do_global_cut then U.cut parameter always handler log_info error event_list
	    	else error,log_info,event_list
	    in 
    ...
    in 
	  let rec aux k (error,log_info,event_list) = 
	    match S.PH.B.PB.CI.Po.K.H.get_bound_on_itteration_number parameter
	    with Some k' when k>=k' -> error,log_info,event_list 
	    | Some _ | None -> 
	      let (error,log_info,event_list') = one_iteration_of_compression (log_info,error,event_list) in 
	      if U.size_of_pretrace event_list' < U.size_of_pretrace event_list then 
      ...
      if weak_compression_on || strong_compression_on then 
	      ...
	      let () = if log_step then Debug.tag logger "\t - blackboard generation" in 
	      ...
	      let error,log_info,causal_story_list = 
	          ...
	          (fun (error,log_info,story_list) observable_id -> 
					    ...
					    let error,trace_before_compression = 
					    	U.causal_prefix_of_an_observable_hit "compression_main, line 2014" parameter handler 
					    		error log_info blackboard enriched_grid observable_id in 
			        let info = match U.get_runtime_info_from_observable_hit observable_id 
	              with 
	              | None -> []
	              | Some info -> 
									...
									[info]
			        in
					    if store_uncompressed_stories then
				        ...
				        in 
				        let error,log_info,trace_without_pseudo_inverse_events = 
									U.cut (do_not_log parameter) always handler log_info error 
										trace_without_pseudo_inverse_events
						    in 
    ...
    let error,weakly_story_table =
      ...
      if store_uncompressed_stories then
	    ...
	    else error,causal_story_table
      else U.create_story_table parameter handler error
	  in 
      ...
      begin 
        ...
        let () = Format.fprintf logger "\t - strong flow compression (%i)@." n_weak_stories in
				...
				let error,(strong_story_table,log_info) =
		 			U.fold_story_table_with_progress_bar logger parameter handler error "strong_compression" 
          ...
          let error,log_info,list = U.compress logger parameter handler error log_info refined_event_list in
             ...
             | _ -> List.fold_left
			       		(fun (error,strong_story_table,log_info) list -> 
								 let list_info = 
								 	List.map (Mods.update_profiling_info (S.PH.B.PB.CI.Po.K.P.copy log_info)) list_info 
								 in  
								 U.store_trace parameter handler error list_info log_info list strong_story_table) 
			       	(error,strong_story_table,log_info) list 
					in 			
					error,(strong_story_table,log_info)
				 ) weakly_story_table (strong_story_table,log_info)
				in U.flatten_story_table parameter handler error strong_story_table 
	    end
      else U.create_story_table parameter handler error
    in 
	  causal_table, causal_story_table, weakly_story_table, strong_story_table 
  end 
  in 
  let error =
    if causal_trace_on then
